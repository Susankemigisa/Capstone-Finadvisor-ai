...
async def _mtn_get_token() -> str:
    ...
    if not settings.MOMO_API_USER or not settings.MOMO_API_KEY:
        raise HTTPException(
            status_code=503,
            detail=(
                "MTN MoMo API credentials are not configured. "
                "Set MOMO_API_USER and MOMO_API_KEY in your .env file. "
                "See: https://momodeveloper.mtn.com to create an API user."
            )
        )
...
async def _mtn_request_to_pay(amount: int, phone: str, ext_id: str, callback_url: str) -> str:
    ...
    # Normalise phone → international format without leading +
    # e.g. 0771234567 → 256771234567,  +256771234567 → 256771234567
    raw = phone.strip().replace(" ", "").replace("-", "")
    if raw.startswith("+"):
        raw = raw[1:]
    if raw.startswith("0"):
        raw = "256" + raw[1:]
    if not raw.startswith("256"):
        raw = "256" + raw
    clean = raw  # e.g. "256771234567"

    # MTN sandbox only accepts one hard-coded test MSISDN.
    # Any real Ugandan number returns HTTP 500 from the sandbox.
    # In sandbox mode we substitute the official test number so the
    ...
    if not is_sandbox and callback_url:
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        ...
        if r.status_code not in (200, 202):
            logger.error(
                ...
                body=r.text[:300],
                ref=ref,
                environment=settings.MOMO_TARGET_ENVIRONMENT,
            )
...
async def poll_payment(reference_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        if p["provider"] == "mtn":
            ...
            logger.debug("mtn_poll_status", ref=reference_id, raw_status=raw, result=str(result)[:200])
            # MTN sandbox auto-approves after ~30s and returns SUCCESSFUL.
            # Production: user approves on phone → SUCCESSFUL, rejects → FAILED.
            if raw in ("SUCCESSFUL", "SUCCESS"):
                ...
                return {"status": "successful", "plan": p["plan"]}
            if raw in ("FAILED", "REJECTED", "EXPIRED", "TIMEOUT"):
                reason = result.get("reason") or result.get("financialTransactionId", "Declined")
