...
function ResetForm() {
  ...
  if (done) {
    return (
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ color: 'var(--text-primary)', fontSize: '14px', marginBottom: '24px' }}>{t('auth.passwordUpdatedMsg')}</p>
  ...
  return (
    <div className="surface" style={{ padding: '32px' }}>
      <p style={{ color: 'var(--text-primary)', fontSize: '13px', marginBottom: '20px' }}>{t('auth.setNewPasswordSubtitle')}</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {!token && (
          <div>
            <label style={{ display: 'block', fontSize: '12px', color: 'var(--text-primary)', marginBottom: '6px', letterSpacing: '0.05em' }}>{t('auth.resetToken')}</label>
        ...
        <div>
          <label style={{ display: 'block', fontSize: '12px', color: 'var(--text-primary)', marginBottom: '6px', letterSpacing: '0.05em' }}>{t('auth.newPasswordLabel')}</label>
  ...
  return (
    <div className="auth-bg">
      ...
      <div style={{ width: '100%', maxWidth: '400px', position: 'relative', zIndex: 1 }} className="fade-in">
        ...
        <Suspense fallback={<div style={{ textAlign: 'center', color: 'var(--text-primary)' }}>{t('common.loading')}</div>}>
          <ResetForm />
        </Suspense>
        <p style={{ textAlign: 'center', marginTop: '20px', color: 'var(--text-primary)', fontSize: '13px' }}>
          <Link href="/login" style={{ color: 'var(--gold-light)', textDecoration: 'none' }}>{t('auth.backToSignIn')}</Link>
        </p>
      </div>
    </div>
  )
