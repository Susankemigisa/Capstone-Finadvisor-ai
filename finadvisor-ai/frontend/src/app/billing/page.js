...
export default function BillingPage() {
  ...
  const validatePhone = (p) => {
    ...
    if (!clean) return t('billing.phoneRequired')
    if (!/^(0|\+256|256)?[1-9]\d{8}$/.test(clean)) return t('billing.invalidPhone')
    if (selectedProvider?.id === 'mtn' && !/^(0|\+256|256)?(77|78|76|79)/.test(clean)) return t('billing.mtnInvalid')
    if (selectedProvider?.id === 'airtel' && !/^(0|\+256|256)?(70|75|74)/.test(clean)) return t('billing.airtelInvalid')
    return ''
  }
  ...
  const handleInitiate = async () => {
    ...
    try {
      ...
      if (data.reference_id) {
        ...
        setErrorMsg(data.detail || t('billing.initiateError'))
        setStep('error')
      }
    } catch (e) {
      setErrorMsg(t('billing.serverError'))
      setStep('error')
    }
    setInitiating(false)
  }
  ...
  const startPolling = (ref) => {
    ...
    pollRef.current = setInterval(async () => {
      ...
      if (count > 40) {
        ...
        setErrorMsg(t('billing.paymentTimeout'))
        setStep('error')
        return
      }
      try {
        ...
        if (data.status === 'successful') {
          ...
          clearInterval(pollRef.current)
          setErrorMsg(data.reason || t('billing.paymentDeclined'))
          setStep('error')
        }
      } catch {}
    }, 3000)
            ...
            <div style={{ background: 'var(--bg-surface)', border: '2px solid var(--gold)', borderRadius: '16px', padding: '32px', textAlign: 'center', marginBottom: '24px' }}>
              ...
              {billingStatus?.expires_at && (
                <div style={{ fontSize: '12px', color: 'var(--text-dim)', marginBottom: '20px' }}>
                  {t('billing.renewsLabel')} {new Date(billingStatus.expires_at).toLocaleDateString('en-UG', { day: 'numeric', month: 'long', year: 'numeric' })}
                </div>
              )}
              <button onClick={async () => { if (!confirm(t('billing.cancelSubConfirm'))) return; await req('/billing/cancel', { method: 'POST' }); setBillingStatus({ is_pro: false, tier: 'free' }) }}
          ...
          {!isPro && step === 'plans' && (
            <>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: '24px', color: 'var(--text-dim)', fontSize: '12px' }}>
                {[t('billing.noCardNeeded'), t('billing.payWithMomoOrAirtel'), t('billing.cancelAnytime')].map(b => <span key={b}>{b}</span>)}
              </div>
            </>
          )}
          ...
          {!isPro && step === 'pay' && (
            <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '32px' }}>
              ...
              <div style={{ background: 'var(--bg-elevated)', borderRadius: '10px', padding: '14px', marginBottom: '20px', fontSize: '12px', color: 'var(--text-secondary)', lineHeight: 1.7 }}>
                ...
                <br />3. {t('billing.howStep3')}
                <br />4. {t('billing.howStep4')}
              </div>
              ...
              <button
                ...
                {initiating ? t('billing.sendingRequest') : `${t('billing.payWithMomo')} UGX ${selectedPlan ? fmt(selectedPlan.amount_ugx) : ''} →`}
              </button>
            </div>
          )}
              ...
              <div style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '16px', lineHeight: 1.7 }}>
                {selectedProvider?.id === 'mtn' ? (
                  <>
                    ...
                    <br />{t('billing.mtnPinPrompt')}
                  </>
                ) : (
                  <>
                    {t('billing.airtelCheck').replace('{amount}', `UGX ${selectedPlan ? fmt(selectedPlan.amount_ugx) : ''}`)}
                    <br />{t('billing.airtelPin')}
                  </>
                )}
              </div>
              <div style={{ fontFamily: 'DM Mono, monospace', fontSize: '11px', color: 'var(--text-dim)', marginBottom: '8px' }}>
                {t('billing.checkingStatus').replace('{secs}', Math.ceil((40 - pollCount) * 3))}
              </div>
              <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '20px' }}>
                {t('billing.referenceLabel')} <span style={{ fontFamily: 'DM Mono, monospace' }}>{referenceId?.slice(0, 8)}…</span>
              </div>
